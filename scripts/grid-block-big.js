	...
	$(gridBlockBig).hover(function(event){
		if(!entered){
			...
			if(event.pageY >= topLine && event.pageY < topGutter){
				$("#content-left-background").velocity({"top": "440px", height: "0px"});
				egress = "up";
			} else if (event.pageY <= bottomLine && event.pageY > bottomGutter){
				$("#content-left-background").velocity({"bottom": "440px", height: "0px"});
				egress = "down";
			} else if (event.pageX >= leftLine && event.pageX < leftGutter){
				$("#content-left-background").velocity({"left": "460px", width: "0px"});
				egress = "left";
			} else if (event.pageX <= rightLine && event.pageX >= rightGutter){
				$("#content-left-background").velocity({"right": "460px", width: "0px"});
				egress = "right";
			}
		} else {
			entered = false;
			switch (egress) {
				case "up":
					$("#content-left-background").velocity({"top": "0px", height: "440px"});
				case "down":
					$("#content-left-background").velocity({"bottom": "0px", height: "440px"});
				case "left":
					$("#content-left-background").velocity({"left": "0px", width: "460px"});
				case "right":
					$("#content-left-background").velocity({"right": "0px", width: "460px"});
			}
		}
